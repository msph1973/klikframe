 ...
 * IDEMPOTENCY_CONFLICT. Rows are tenant-tagged with `workspace_id`
 * (nullable because onboarding runs pre-workspace) and carry the §6
 * composite unique `(workspace_id, id)` so future rows can be FK targets.
 */
export const idempotencyRequests = pgTable(
  ...
  (table) => [
    unique("idempotency_requests_scope_key").on(
      table.principalId,
      table.route,
      table.resourceId,
      table.key,
